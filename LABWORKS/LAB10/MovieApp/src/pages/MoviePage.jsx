  ...
  return (
   ...
   className=' text-center font-bold text-[50px]'>
    ...
    <div className='w-11/12 p-2 mx-auto mt-20 flex flex-row gap-5  '>
        ...
        value={movieName}
        type="text" name="" id="" placeholder='Search for the Movie '/>
        <button 
        onClick={searchMovieHandler}
        className='px-[16px] py-[8px] bg-[#154210] text-white font-semibold rounded-md' 
    ...
    <div className='w-11/12 mx-auto my-5 p-2  flex flex-row-reverse  gap-2 items-start'>
      <div className='flex flex-col w-[60%] '>
        <div className='flex flex-row items-center justify-between'>
          <div>
          <div className='text-[60px] font-extrabold flex-1 text-[#154210]'>
            {movieData?.Title}
          </div>
          ...
          <div>Release Date : <strong className='text-[#154210]'>{movieData?.Released}</strong></div>
          ...
          <div className=' mr-5 w-[100px]  flex-col h-[100px] flex justify-center text-center items-center  border-[#531616]'>
            <span className='text-[30px] font-bold'>{movieData?.imdbRating}</span>
          </div>
        </div>

       
          <div className='flex flex-row gap-2  p-2 rounded-[50px]'>
              {movieData?.Actors?.split(",").map((node,i)=>(
                <div key={i} className='px-2 py-1 bg-[#154210] rounded-[50px] text-white font-semibold'>{node}</div>
              ))
